...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('mousedown mouseup', '.flowchart-operator-connector', function (e) {
                var $this = $(this);
            ...
            this.objs.layers.links.on('mouseout', '.flowchart-link', function () {
             ...
             this.objs.layers.operators.on('mouseover', '.flowchart-operator-connector-delete', function () {
                self.colorizeLink($(this).data("link_id"), '#ff0000');
                $(this).show();
            });
            this.objs.layers.operators.on('mouseout', '.flowchart-operator-connector-delete', function () {
                var l_id = $(this).data('link_id');
                if(typeof(self.data.links[l_id])!=="undefined"){
                    self._connecterMouseOut(l_id);
                }
                $(this).hide();
            });
            this.objs.layers.operators.on('click', '.flowchart-operator-connector-delete', function (e) {                
                e.preventDefault();
                e.stopPropagation();
                $(this).hide();
                self._deleteLink($(this).data("link_id"),false);
                self.objs.layers.links.trigger("click");
                return false;
            });
        ...
        _drawLink: function (linkId) {
            ...
            var $_deleteLink =  fromSmallConnector.next();
            $_deleteLink.data('link_id', linkId);

            linkData.internal.els.fromSmallConnector = fromSmallConnector;
            linkData.internal.els.deleteLink = $_deleteLink;
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            if(connectorKey.indexOf("output_")>-1){
                var $operator_connector_delete = $('<div class="flowchart-operator-connector-delete" title="Delete link" style="display:none"><div></div></div>');
                    $operator_connector_delete.appendTo($operator_connector);
            }
        ...
        colorizeLink: function (linkId, color, showRemoveLink) {
            var linkData = this.data.links[linkId];
            ...
            if(typeof(showRemoveLink)!=="undefined"){
                if(showRemoveLink){
                    linkData.internal.els.deleteLink.show();
                }
                else{
                    linkData.internal.els.deleteLink.hide();
                }
            }
        },

        uncolorizeLink: function (linkId) {
            this.colorizeLink(linkId, this.getLinkMainColor(linkId),false);
        },

        _connecterMouseOver: function (linkId) {
            if (this.selectedLinkId != linkId) {
                this.colorizeLink(linkId, this._shadeColor(this.getLinkMainColor(linkId), -0.4),true);
            }
        },
        ...
        unselectLink: function () {
            if (this.selectedLinkId != null) {
                ...
                this.uncolorizeLink(this.selectedLinkId, this.options.defaultSelectedLinkColor,false);
