        ...
        function (template, chart, contactsView, jsPDF) {
            ...
            return Backbone.View.extend({
                ...
                render: function () {
                    ...
                    this.$el.find('.chart-pending-views strong').on('click', function (ev) {
                        that.pendingViews(ev);
                        return false;
                    })
                    this.$el.find('.chart-sent-views strong').on('click', function (ev) {
                        that.sentViews(ev);
                        return false;
                    })

                    if (/^((?!chrome).)*safari/i.test(navigator.userAgent)) { // check if browser is safari
                        this.$('#imgLogo').load(function () {
                            that.completeImageLoad1 = true;
                        });
                        this.$('#imgCon').load(function () {
                            that.completeImageLoad2 = true;
                        });
                ...
                openContacts: function (ev) {
                    ...
                    if ($(ev.target).hasClass("download-sub-csv")) {
                        this.trigger("countsDownloadClick", ev);
                        $("body>.tooltip").remove();
                        return false;
                    }
                ...
                sentViews: function (ev) {
                    ...
                    if (this.active_ws.find(".sent-views").parents('li').hasClass('active'))
                        return;
                    this.clearHTML();
                ...
                pendingViews: function (ev) {
                    this.active_ws = this.$el.parents(".ws-content.active");
                    if (this.active_ws.find(".pending-views").parents('li').hasClass('active'))
                        return;
                    this.clearHTML();
                ...
                getImgData: function () {
                    ...
                    require(['reports/summary/collections/links'], function (Links) {
                        ...
                        if (pending)
                            doc.text(120, 32, "Pending: " + pending);
                        if (imgData) {
