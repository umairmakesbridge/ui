        ...
        function(template) {
            ...
            return Backbone.View.extend({
                ...
                render: function() {
                   ...
                   // this.$("#ddlworksheet").chosen({no_results_text: 'Oops, nothing found!', width: "200px", disable_search: "true"});
                    console.log(this.options.spreadSheet);
                   // this.getWorksheet();
                ...
                loadWorksheet:function(ev){
                    ...
                    var data = {
                     ...
                     var that = this;
                     that.$el.find("#ddlworksheet").html("<option value='0'>Select worksheet</option>");
                     $.getJSON(URL, data)
                         ...
                         var sheet = json.worksheetList[0];
                           _.each(sheet, function(elems, idx) {
                               _.each(elems, function(elem, idx) {
                                   that.$el.find("#ddlworksheet").append("<option value='"+elem.id+"'>"+elem.title+"</option>");
                                });
                           });
                           that.$el.find("#ddlworksheet").show();
                ...
                setGoogleData: function() {
                    if (this.parent.editImport || (this.parent.Import_page != null)) {
                        ...
                        if (recipient_obj.filterType === "all") {
                            this.$(":radio[value=importall]").iCheck('check');
                            this.$('#panel_2').slideDown();
                        }
                    } else {
                        this.$(":radio[value=importall]").iCheck('check');
                    }
