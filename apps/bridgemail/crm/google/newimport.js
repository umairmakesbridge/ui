define(['views/common/wizard','text!crm/google/html/newimport.html','moment','jquery.bmsgrid','jquery.searchcontrol','jquery.highlight','bms-addbox','jquery.chosen','jquery-ui','jquery.icheck'],
        ...
        return Backbone.View.extend({                                
                    ...
                    if( this.improtLoaded==false){
                        ...
                        require(["crm/google/import"],_.bind(function(page){    
                            that.Import_page = new page({
                                page:that,spreadSheet:that.spreadSheets
                            })
                            this.app.showLoading(false,this.$(".step2"));
                            that.$(".step2 .netsuite_campaigns").remove();
                            that.$(".step2").append(that.Import_page.$el);                       
                        },this));
                    }
                },
                initStep3:function(){
                ...
                createListTable:function(xhr){                
                    ...
                    $.each(camp_list_json.lists[0], _.bind(function(index, val) {    
                       if(val[0]["isBounceSupressList"]==="false"){
                ...
                loadData:function(data){
                     if(data){
                        ...
                        var list_ = this.$("#import-list-grid tr[checksum='"+data.checkSum+"']").find(".name-type h3");
                        if(list_.length){
